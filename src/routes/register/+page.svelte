...
<main>
	...
	<form method="POST">
		...
		{#if form?.missingEmail}The email field is required{/if}
		{#if form?.missingPassword}The password field is required{/if}
		{#if form?.usernameTaken}This username is already taken{/if}
		{#if form?.emailInUse}There's an account associated with this email already{/if}

		{#await status}
			<button disabled>CREATE ACCOUNT</button>
		{:then status}
			<button disabled={status.taken || status.invalid || !email || !password}>
				CREATE ACCOUNT
			</button>
		{/await}
	</form>
